...
MODEL_OUT = Path(
    "routing/visual_rescue_router_budget50_with_type.joblib"
)
...
X_train = vectorizer.fit_transform(
    train["router_text"].astype(str)
)

X_val = vectorizer.transform(
    val["router_text"].astype(str)
)

X_test = vectorizer.transform(
    test["router_text"].astype(str)
)
...
X_tv = final_vectorizer.fit_transform(
    trainval["router_text"].astype(str)
)
